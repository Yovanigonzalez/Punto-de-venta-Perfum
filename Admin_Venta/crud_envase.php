...
// Modificar las consultas SQL para incluir las columnas de la tabla 'envase'
if ($search_query) {
    $sql_count = "SELECT COUNT(*) as total FROM envase WHERE envase LIKE '%$search_query%' OR categoria LIKE '%$search_query%'";
    $sql = "SELECT * FROM envase WHERE envase LIKE '%$search_query%' OR categoria LIKE '%$search_query%' LIMIT $offset, $registros_por_pagina";
} else {
    $sql_count = "SELECT COUNT(*) as total FROM envase";
    $sql = "SELECT * FROM envase LIMIT $offset, $registros_por_pagina";
}
...
<body class="hold-transition sidebar-mini">
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                <?php foreach ($envases as $envase) { ?>
                                                    <tr>
                                                        <td><?php echo $envase['envase']; ?></td>
                                                        <td><?php echo $envase['precio']; ?></td>
                                                        <td><?php echo $envase['gramos']; ?></td>
                                                        <td><?php echo $envase['categoria']; ?></td>
                                                        <td>
                                                            <!-- Botón para abrir el modal de confirmación -->
                                                            <button class="btn btn-danger" data-toggle="modal" data-target="#confirm-delete-<?php echo $envase['id']; ?>">Eliminar</button>
                                                            <a class="btn btn-primary" href="editar_envase.php?id=<?php echo $envase['id']; ?>">Editar</a>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
    ...
    ?>
        ...
        <?php foreach ($envases as $envase) { ?>
            ...
            <div class="modal fade" id="confirm-delete-<?php echo $envase['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-danger" onclick="eliminarEnvase(<?php echo $envase['id']; ?>)">Eliminar</button>
                        </div>
                    </div>
                </div>
            </div>
...
function eliminarEnvase(id) {
    ...
    window.location.href = `eliminar_envase.php?id=${id}`;
}
</script>
</body>
</html>
